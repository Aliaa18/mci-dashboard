...
export default function Categories() {
 ...
 let [formData , setFormData] = useState({
  ...
  const handleChange = (category = null) =>{
   if(category){
     seteditMode(true)
 ...
 const handleClose = () => {
   ...
   const columns = [
     ...
     { field: 'subcategories', headerName: 'Subcategories', width: 300 }, 
...
async function onSunmit (values){
   if(editMode===true ){
      console.log(values);
    await updateCategory( values.slug ,values)
    setTimeout(()=>{
     closeDialog()
    } , 1000)
  ...
  try {
    ...
    const transData = data?.categories.map((category) => ({
      slug: category.slug,
      id: category._id,
      name: category.name,
       ...
       buttons: [
         {
           ...
           onClick:async () => {
             try {
               let { data } = await axios.delete(`https://mcishop.vercel.app/api/v1/categories/${slug}`);
    ...
    async function addCategory(data) {
     try {
       ...
       let res = await axios.post('https://mcishop.vercel.app/api/v1/categories' , {
        name:data.name
       } , {
         headers:{
   ...
   // console.log(data.product);
    ...
    async function updateCategory(slug , data ) {
     try {
       let res = await axios.put(`https://mcishop.vercel.app/api/v1/categories/${slug}` , {
        name:data.name
       } , {
         headers:{
          "Content-Type": "application/json"
   ...
   return <>
    ...
    <Dialog
       ...
       visible={visible}
       onClose={closeDialog}
       title={editMode===true ? "Update category " : "Add new category"}
       footer={[
  ...
  >
    ...
    {selectedRow && (
      <div>
        ...
        {/* Subcategories List */}
        <Typography className="mt-3"><Link to={`/categories/${selectedRow.slug}`}>Subcategories</Link> </Typography>
